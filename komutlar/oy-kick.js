...
module.exports.run = async (bot, message, args) => {
  ...
  var sumsum = new Discord.RichEmbed()
            ...
            .addField("**Oylama Tamamlandı!**", "═════════════════════════\n" +
                                          ":white_check_mark:  **| Toplam Oy (**\`Evet\`**)** ➠ " + `${YES_Count-1}\n` +
                                          ":x:**| Toplam Oy (**\`Hayır\`**)** ➠ " + `${NO_Count-1}\n` +
                                          "═════════════════════════\n" +
  ...
  if(YES_Count >= 4 && YES_Count > NO_Count){
    ...
    kickmember.kick().then(member => {
      message.reply(`:beginner: **|** ${member.user.username} **Adlı Kullanıcı Başarıyla Oy Kick Sistemi İle Tekmelendi.**`)
    })
  }else{
