...
exports.run = async function(client, message, args) {
 ...
 //if (!message.member.hasPermission("MANAGE_MESSAGES")) return message.reply(`Bu komutu kullanabilmek için **Mesajları Yönet** iznine sahip olmalısın!`);
  ...
  var x = args[1]
  if (!u) return message.reply("İstediğiniz Birisinin mesajını silmem için  etiketlemelisin!")
 ...
 var fetched = await message.channel.fetchMessages({limit: x})
  ...
  message.channel.send(`**${u.tag}** adlı kullanıcının **${x}** adet mesajı başarıyla silindi! :zap: `)
